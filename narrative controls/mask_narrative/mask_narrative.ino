/* -----------------------------------------------------------------------------
 *
* File:        mask_narrative.ino
 *
 * Author:Rambod Taherian, Kourosh Ghahramani
 * Date:        2025-08-23
 *
 * Description:
 * This project extends the expressive robotic mask system 
 * with a new Narration Mode, allowing synchronized jaw motion playback 
 * alongside spoken audio or pre-recorded tracks. The jaw servo follows a 
 * pre-generated frame sequence (jawTrack[]), streamed at 50 Hz 
 * under FreeRTOS, producing realistic mouth movement during narration.
 *
 * Key Narration Features:
 *   - ESP-NOW message type (MSG_NARRATE) with commands:
 *       NARR_START, NARR_STOP, NARR_TOGGLE
 *   - Dedicated TaskNarrate running at 50 Hz for smooth jaw updates
 *   - Jaw motion data stored in jawTrack[] with frame indexing
 *   - Integration with existing TaskSlew for clamp/smoothing
 *   - Seamless transition between narration and other modes
 *
 * Other System Features (unchanged from base project):
 *   - Free Play & Game modes with randomized face-matching
 *   - Face Preview (min/mid/max pose slewing per expression)
 *   - Randomizer mode for expressive servo testing
 *   - Position persistence with NVS
 * ----------------------------------------------------------------------------- */

// ─────────────────────────────────────────────────────────────────────────────
// Includes: core Arduino + ESP32 + drivers + CRC + local jaw track
// ─────────────────────────────────────────────────────────────────────────────
#include <Arduino.h>
#include <Wire.h>
#include <WiFi.h>
#include <esp_now.h>
#include <Preferences.h>
#include <Adafruit_PWMServoDriver.h>
#include <esp_crc.h>
#include <limits.h>
#include "jaw_track.h"   // generated from Colab (jawTrack[], jawFrames)

// ─────────────────────────────────────────────────────────────────────────────
// Servo/channel configuration
// N_SERVO: total number of PCA9685 channels used for the mask
// ─────────────────────────────────────────────────────────────────────────────
#define N_SERVO        14

/* I2C / PCA9685 */
#define SDA_PIN        5      // I2C SDA pin to PCA9685
#define SCL_PIN        4      // I2C SCL pin to PCA9685
#define SERVO_FREQ     50     // 50Hz servo PWM
#define MIN_US_GLOBAL  500    // global lower bound (safety)
#define MAX_US_GLOBAL  2400   // global upper bound (safety)
#define MAX_STEP_US    20     // slew limiter per 20ms tick (soften motion)

#define USE_OE         1      // if 1, use PCA9685 /OE pin to enable outputs
#if USE_OE
#define OE_PIN        18      // PCA9685 OE pin (active LOW)
#endif

/* Controller MAC (update if needed) */
static const uint8_t CONTROLLER_MAC[6] = {0xFC,0x01,0x2C,0xDB,0xF4,0x00};
static uint8_t ctlMac[6] = {0};         // last sender MAC remembered
static bool    ctlMacKnown = false;     // becomes true after first packet

// Narration (jaw tracking) flags updated via MSG_NARRATE
static volatile int     narrIdx = 0;    // index into jawTrack[]
static volatile bool    narrOn  = false;

/* Protocol */
// Top-level message type IDs exchanged over ESP-NOW
enum : uint8_t { MSG_MOVE=1, MSG_SYNC_REQ=2, MSG_SYNC_RESP=3, MSG_MODE=4, MSG_RANDOM=5, MSG_GAME=6 };
enum : uint8_t { MODE_TOGGLE=1, MODE_SET_FREE=2, MODE_SET_GAME=3 };
enum : uint8_t {
  GAME_CFG=1, GAME_START=2, GAME_ABORT=3, GAME_STATE=4, GAME_FACE_DONE=5, GAME_RESULT=6,
  GAME_PREVIEW=7 // NEW: controller requests preview pose application
};
enum : uint8_t { PREV_MID=0, PREV_MIN=1, PREV_MAX=2 };
enum : uint8_t { MSG_NARRATE = 7 };     // new top-level type for narration control
enum : uint8_t { NARR_START=1, NARR_STOP=2, NARR_TOGGLE=3 };

// Small control pkt for narration start/stop/toggle
struct __attribute__((packed)) PktNarrate { uint8_t type, cmd; uint16_t crc; };

/* Per-servo limits */
// Each channel has its own safe [min,max] µs window
struct Limits { int16_t minUs, maxUs; };
static Limits lim[N_SERVO] = {
  { 960,1065} /*0  L brow V*/,
  {1041,2248} /*1  L brow A*/,
  {2015,2160} /*2  R brow V*/,
  {1006,2111} /*3  R brow A*/,
  {1039,1600} /*4  R eye X*/,
  { 890,1510} /*5  R eye Y*/,
  { 981,2300} /*6  R eyelid*/,
  {1557,2180} /*7  L eye X*/,
  {1280,1871} /*8  L eye Y*/,
  {1139,2400} /*9  L eyelid*/,
  {1216,1750} /*10 Jaw*/,
  { 660,2400} /*11 R mouth*/,
  { 500,2060} /*12 L mouth*/,
  { 500,2400} /*13 Extra*/
};

/* Optional names */
// Human-readable channel labels (for logs/prints/debug)
static const char* CH_NAME[N_SERVO] = {
  "L brow V","L brow A","R brow V","R brow A",
  "R eye X","R eye Y","R eyelid","L eye X","L eye Y","L eyelid",
  "Jaw","R mouth","L mouth","Extra"
};

/* Faces (using U() and IGN)
   Each Face defines acceptable [min,max] ranges per channel.  IGN marks "don't care". */
struct Range { int16_t minUs, maxUs; };
struct Face  { const char* name; Range ch[N_SERVO]; };
#define IGN {INT16_MAX, INT16_MIN}                     // "ignore this channel" sentinel (min>max)
#define U(us,tol) { int16_t((us)-(tol)), int16_t((us)+(tol)) } // symmetric window around a center

/* >>> You can tweak these safely; kept within your channel limits <<< */
static Face faces[] = {

/* 0: Neutral-ish "Poker" baseline */
{ "Poker", {
  U(1045,55)  /*0 L brow V*/,
  U(1420,100) /*1 L brow A*/, 
  U(2045,35)  /*2 R brow V*/, 
  U(1510,80) /*3 R brow A*/,
  U(1290,230)  /*4 R eye X*/, 
  U(1210,170) /*5 R eye Y*/, 
  U(1560,300) /*6 R eyelid*/, 
  U(1880,230) /*7 L eye X*/,
  U(1620,170) /*8 L eye Y*/, 
  U(1670,300) /*9 L eyelid*/, 
  U(1260,60)  /*10 Jaw*/, 
  U(1310,150) /*11 R mouth*/,
  U(1290,150) /*12 L mouth*/, 
  IGN         /*13 Extra*/
}},

/* 1: Happy mouth corners up; brows and lids permissive */
{ "Happy", {
  U(1045,55)  /*0 L brow V*/, 
  U(1600,200) /*1 L brow A*/, 
  U(2045,35)  /*2 R brow V*/, 
  U(1345,200) /*3 R brow A*/,
  IGN /*4 R eye X*/, 
  IGN /*5 R eye Y*/, 
  U(1560,300) /*6 R eyelid*/,
  IGN /*7 L eye X*/,
  IGN /*8 L eye Y*/, 
  U(1670,300) /*9 L eyelid*/, 
  IGN /*10 Jaw*/, 
  U( 922,330) /*11 R mouth*/,
  U(1730,330) /*12 L mouth*/, 
  IGN         /*13 Extra*/
}},

/* 2: Angry brows down/angled; mouth corners down */
{ "Angry", {
  U(1045,55)  /*0 L brow V*/, 
  U(1158,117) /*1 L brow A*/, 
  U(2045,35)  /*2 R brow V*/, 
  U(1871,240) /*3 R brow A*/,
  IGN /*4 R eye X*/, 
  IGN /*5 R eye Y*/, 
  U(1560,300) /*6 R eyelid*/, 
  IGN /*7 L eye X*/,
  IGN /*8 L eye Y*/, 
  U(1670,300) /*9 L eyelid*/, 
  IGN /*10 Jaw*/, 
  U( 1890,510) /*11 R mouth*/,
  U(855,355) /*12 L mouth*/, 
  IGN         /*13 Extra*/
}},

/* 3: Sad brows and corners */
{ "Sad", {
  U(1045,55)  /*0 L brow V*/, 
  U(1600,200) /*1 L brow A*/, 
  U(2045,35)  /*2 R brow V*/, 
  U(1345,200) /*3 R brow A*/,
  IGN /*4 R eye X*/, 
  IGN /*5 R eye Y*/, 
  U(1560,300) /*6 R eyelid*/, 
  IGN /*7 L eye X*/,
  IGN /*8 L eye Y*/, 
  U(1670,300) /*9 L eyelid*/, 
  U(1260,60) /*10 Jaw*/, 
  U( 1890,510) /*11 R mouth*/,
  U(855,355) /*12 L mouth*/, 
  IGN         /*13 Extra*/
}},

// Example alternative face kept commented for future tuning
// /* 4 */
// { "Confused", { ... }},

/* 5: Shocked: high lids/jaw open, permissive eyes */
{ "Shocked", {
  U(983,25)  /*0 L brow V*/, 
  U(1420,80) /*1 L brow A*/, 
  U(2115,45)  /*2 R brow V*/, 
  U(1510,80) /*3 R brow A*/,
  U(1290,230)  /*4 R eye X*/, 
  U(1210,170) /*5 R eye Y*/,
  U( 1192,211) /*6 R eyelid*/, 
  U(1880,230) /*7 L eye X*/,
  U(1620,170) /*8 L eye Y*/,  
  U(2127,273) /*9 L eyelid*/, 
  U(1700,50)  /*10 Jaw*/, 
  U(1310,250) /*11 R mouth*/,
  U(995,300) /*12 L mouth*/, 
  IGN         /*13 Extra*/
}},

// Another Poker variant (disabled)
// /* 6 */
// { "Poker", { ... }},
};

static const int NUM_FACES = sizeof(faces)/sizeof(faces[0]); // total defined faces

/* Globals */
// PCA9685 driver at I2C address 0x40
Adafruit_PWMServoDriver pwm(0x40);
// NVS store for last positions
Preferences prefs;

// Quick message struct for TaskRx queue
struct Msg { uint8_t len; uint8_t src[6]; uint8_t bytes[64]; };
static QueueHandle_t q; // FreeRTOS queue for incoming ESP-NOW packets

// Servo position targets/current/last-printed (µs)
static volatile int16_t targetUs[N_SERVO];   // where we want to go
static int16_t posUs[N_SERVO];               // where we currently are
static int16_t lastPrintedUs[N_SERVO];       // for debug spam throttle

// Persistence flags (periodic save to NVS)
static volatile bool saveDirty=false;
static uint32_t lastSaveMs=0;

/* Game state */
// Mode flags (free/game), round progress, and result reporting
static bool     inGameMode = false;
static bool     gameActive = false;
static bool     gameResultSent = false;

static uint8_t  orderIdx = 0;        // index into randomized face order
static uint8_t  order[32];           // randomized indices buffer
static uint32_t faceStartMs = 0;     // start time of current face
static uint16_t stableCount = 0;     // consecutive frames within tolerance

// per round configuration (settable via GAME_CFG)
static uint8_t  gameFaces = 3;       // how many faces per round
static uint32_t faceMs    = 30000;   // time allowed per face (ms)

#define STABLE_MATCH_FRAMES   5      // number of consecutive good frames to "lock" a face
#define GAME_CHECK_PERIOD_MS  20     // TaskGame loop period (ms)

/* RANDOM trigger from controller */
static volatile bool randomTrigger = false;

/* Preview state (for GAME_PREVIEW) */
static volatile bool previewActive = false;
static volatile uint8_t previewFace = 0;      // which face to preview
static volatile uint8_t previewMode = PREV_MID; // MID/MIN/MAX

/* Helpers */

// Convert microseconds to PCA9685 12-bit tick count (0..4095) for SERVO_FREQ
static inline uint16_t us2cnt(int us){
  return (uint16_t)(us / (1e6f / (SERVO_FREQ * 4096.0f)));
}

// Clamp per-channel within both local limits and global safety bounds
static inline int16_t clampServoUs(uint8_t i, int16_t us){
  int16_t a = constrain(us, lim[i].minUs, lim[i].maxUs);
  return constrain(a, MIN_US_GLOBAL, MAX_US_GLOBAL);
}

// Ensure a peer exists in ESP-NOW peer list (no encryption)
static void ensurePeer(const uint8_t mac[6]){
  if (!esp_now_is_peer_exist(mac)) {
    esp_now_peer_info_t peer{}; memcpy(peer.peer_addr, mac, 6);
    peer.ifidx=WIFI_IF_STA; peer.channel=0; peer.encrypt=false;
    esp_now_add_peer(&peer);
  }
}

// Persist positions to NVS with debounce (800ms after last change)
static void savePositionsIfNeeded(){
  uint32_t now=millis();
  if (saveDirty && (now-lastSaveMs)>800){
    prefs.putBytes("pos", posUs, sizeof(posUs));
    lastSaveMs=now; saveDirty=false;
  }
}

// Random integer in [mn..mx], handles reversed bounds defensively
static inline int16_t randIn(int16_t mn, int16_t mx){
  if (mx < mn) return mn;
  uint32_t r = esp_random();
  return (int16_t)(mn + (r % (uint32_t)(mx - mn + 1)));
}

// Fisher–Yates shuffle over [0..n-1], wrapping by NUM_FACES
static void shuffleOrder(uint8_t* arr,int n){
  for(int i=0;i<n;i++) arr[i]=i % NUM_FACES;
  for(int i=n-1;i>0;--i){ uint32_t r=esp_random(); int j=r%(i+1); uint8_t t=arr[i]; arr[i]=arr[j]; arr[j]=t; }
}

// Check if all servo positions fit within face ranges (IGN channels skipped)
static bool faceMatched(const Face& f){
  for(uint8_t i=0;i<N_SERVO;++i){
    int16_t mn=f.ch[i].minUs, mx=f.ch[i].maxUs;
    if (mn<=mx){
      int16_t v=posUs[i];
      if (v<mn || v>mx) return false;
    }
  }
  return true;
}

/* GAME send helper */
// Sends a compact game packet (type, cmd, a, b, crc) to controller
static void sendGame(uint8_t cmd, uint8_t a=0, uint8_t b=0){
  if (!ctlMacKnown) return;
  ensurePeer(ctlMac);
  struct __attribute__((packed)) PktGame { uint8_t type, cmd, a, b; uint16_t crc; };
  PktGame g{}; g.type=MSG_GAME; g.cmd=cmd; g.a=a; g.b=b;
  g.crc=esp_crc16_le(0,(uint8_t*)&g,sizeof(g)-2);
  esp_now_send(ctlMac,(uint8_t*)&g,sizeof(g));
}

/* GAME control */
// Initialize a new round: shuffle faces, reset timers/counters, push initial state
static void gameStart(){
  uint8_t n = gameFaces;
  if (n > NUM_FACES) n = NUM_FACES;
  if (n < 1) n = 1;

  shuffleOrder(order, n);
  orderIdx=0; faceStartMs=millis(); stableCount=0;
  gameActive=true; gameResultSent=false;

  uint32_t s0 = faceMs / 1000U;
  if (s0 > 255) s0 = 255;
  sendGame(GAME_STATE, (uint8_t)s0, order[orderIdx]);
}

// End round with a win/lose result (sent once)
static void gameStop(bool won){
  if (gameActive) gameActive=false;
  if (!gameResultSent){
    sendGame(GAME_RESULT, won?1:0, 0);
    gameResultSent=true;
  }
}

/* Apply preview pose */
// For each non-IGN channel of Face[idx], choose MIN/MID/MAX and write targetUs[]
static void applyPreview(uint8_t idx, uint8_t mode){
  if (idx >= NUM_FACES) idx = NUM_FACES - 1;
  const Face& F = faces[idx];

  for (uint8_t i=0;i<N_SERVO;++i){
    int16_t mn = F.ch[i].minUs;
    int16_t mx = F.ch[i].maxUs;

    if (mn > mx){
      // IGN: leave target unchanged
      continue;
    }
    int32_t mid = (int32_t)mn + (int32_t)mx;
    mid /= 2;

    int16_t goal = (mode==PREV_MIN) ? mn : (mode==PREV_MAX) ? mx : (int16_t)mid;
    goal = clampServoUs(i, goal);
    targetUs[i] = goal;  // Slew loop will glide
  }
}

/* ESP-NOW RX ISR → queue */
// Minimal ISR body: copy data/mac into Msg and push to RX queue
static void onRx(const esp_now_recv_info_t* info, const uint8_t* data, int len){
  if (!info || len<=0 || len>(int)sizeof(((Msg*)0)->bytes)) return;
  Msg m; m.len=(uint8_t)len; memcpy(m.src, info->src_addr, 6); memcpy(m.bytes, data, len);
  BaseType_t hpw=pdFALSE; xQueueSendFromISR(q,&m,&hpw); if(hpw) portYIELD_FROM_ISR();
}

/* Tasks */

// TaskRx: parses queued messages and updates state/targets accordingly
static void TaskRx(void*){
  Msg m;
  for(;;){
    if (xQueueReceive(q,&m,portMAX_DELAY)!=pdTRUE) continue;
    memcpy(ctlMac, m.src, 6);
    ctlMacKnown = true;
    ensurePeer(ctlMac);

    if (m.len<1) continue;
    uint8_t type=m.bytes[0];

    if (type==MSG_MOVE){
      // Controller direct move: set targetUs[idx] to us
      if (m.len < (int)sizeof(uint8_t)+ (int)sizeof(uint8_t)+ (int)sizeof(int16_t)*2 + (int)sizeof(uint16_t)) continue;
      struct __attribute__((packed)) PktMove { uint8_t type, idx; int16_t us, xOff; uint16_t crc; };
      const PktMove* p=(const PktMove*)m.bytes;
      bool ok = (p->idx < N_SERVO) &&
                (esp_crc16_le(0,(uint8_t*)p,sizeof(PktMove)-2)==p->crc);
      if (ok && p->us!=-1){
        targetUs[p->idx]=clampServoUs(p->idx, p->us);
      }

    } else if (type==MSG_SYNC_REQ){
      // Controller asks for current positions
      if (m.len != (int)sizeof(uint8_t)+ (int)sizeof(uint16_t)) continue;
      struct __attribute__((packed)) PktSyncReq { uint8_t type; uint16_t crc; };
      const PktSyncReq* rq=(const PktSyncReq*)m.bytes;
      bool ok=(esp_crc16_le(0,(uint8_t*)rq,sizeof(PktSyncReq)-2)==rq->crc);
      if (ok){
        ensurePeer(m.src);
        struct __attribute__((packed)) PktSyncResp{ uint8_t type; int16_t pos[N_SERVO]; uint16_t crc; };
        PktSyncResp r{}; r.type=MSG_SYNC_RESP;
        for(int i=0;i<N_SERVO;++i) r.pos[i]=posUs[i];
        r.crc=esp_crc16_le(0,(uint8_t*)&r,sizeof(r)-2);
        esp_now_send(m.src,(uint8_t*)&r,sizeof(r));
      }

    } else if (type==MSG_MODE){
      // Switch between Free/Game modes (or toggle)
      if (m.len != (int)sizeof(uint8_t)*3 + (int)sizeof(uint16_t)) continue;
      struct __attribute__((packed)) PktMode { uint8_t type, cmd, arg; uint16_t crc; };
      const PktMode* pm = (const PktMode*)m.bytes;
      bool ok = (esp_crc16_le(0,(uint8_t*)pm,sizeof(PktMode)-2)==pm->crc);
      if (ok){
        if (pm->cmd==MODE_SET_FREE){
          inGameMode=false;
          gameStop(false);
        } else if (pm->cmd==MODE_SET_GAME){
          inGameMode=true;
          gameStop(false);
        } else if (pm->cmd==MODE_TOGGLE){
          inGameMode = !inGameMode;
          if (!inGameMode) gameStop(false);
        }
      }

    } else if (type==MSG_GAME){
      // Game commands (CFG/START/ABORT/PREVIEW)
      if (m.len < (int)sizeof(uint8_t)*4 + (int)sizeof(uint16_t)) continue;
      struct __attribute__((packed)) PktGame { uint8_t type, cmd, a, b; uint16_t crc; };
      const PktGame* g = (const PktGame*)m.bytes;
      bool ok = (esp_crc16_le(0,(uint8_t*)g,sizeof(PktGame)-2)==g->crc);
      if (!ok) continue;

      if (g->cmd==GAME_CFG){
        // Configure faces per round and seconds per face
        uint8_t wantFaces = g->a;
        uint8_t wantSec   = g->b;
        if (wantFaces < 1) wantFaces = 1;
        if (wantFaces > NUM_FACES) wantFaces = NUM_FACES;
        if (wantSec   < 5) wantSec   = 5;
        if (wantSec   > 240) wantSec = 240;
        gameFaces = wantFaces;
        faceMs    = (uint32_t)wantSec * 1000U;

      } else if (g->cmd==GAME_START){
        // Only start if we are in game mode
        if (inGameMode) gameStart();

      } else if (g->cmd==GAME_ABORT){
        // Immediate stop -> lose state reported
        gameStop(false);

      } else if (g->cmd==GAME_PREVIEW){
        // Apply preview (if no active game to avoid contention)
        if (!gameActive){ // do not fight the running game
          previewActive = true;
          previewFace = g->a;
          previewMode = g->b;  // 0=MID,1=MIN,2=MAX
          applyPreview(previewFace, previewMode);
          Serial.printf("Preview: face=%u mode=%u\n", (unsigned)previewFace, (unsigned)previewMode);
        }
      }

    } else if (type==MSG_RANDOM){
      // Randomizer trigger
      if (m.len != (int)sizeof(uint8_t) + (int)sizeof(uint16_t)) continue;
      struct __attribute__((packed)) PktRandom { uint8_t type; uint16_t crc; };
      const PktRandom* pr = (const PktRandom*)m.bytes;
      bool ok = (esp_crc16_le(0,(uint8_t*)pr,sizeof(PktRandom)-2)==pr->crc);
      if (ok){
        randomTrigger = true;
      }
    } else if (type==MSG_NARRATE){
  // Narration control: start/stop/toggle jaw tracking playback
  if (m.len != (int)sizeof(PktNarrate)) continue;
  const PktNarrate* pn = (const PktNarrate*)m.bytes;
  bool ok = (esp_crc16_le(0,(uint8_t*)pn,sizeof(PktNarrate)-2)==pn->crc);
  if (!ok) continue;
  if      (pn->cmd==NARR_START)  StartNarration();
  else if (pn->cmd==NARR_STOP)   StopNarration();
  else if (pn->cmd==NARR_TOGGLE) ToggleNarration();

  }
     } 
}

// TaskSlew: 50Hz smoothing/clamping writer that nudges posUs toward targetUs
static void TaskSlew(void*){
  const TickType_t period=pdMS_TO_TICKS(20);
  TickType_t next=xTaskGetTickCount();
  for(;;){
    bool changed=false;
    for(uint8_t i=0;i<N_SERVO;++i){
      int16_t t=targetUs[i], p=posUs[i], d=t-p;
      if (d){
        int16_t step=d;
        if(step> MAX_STEP_US) step= MAX_STEP_US;
        if(step<-MAX_STEP_US) step=-MAX_STEP_US;
        p=clampServoUs(i, (int16_t)(p+step));
        posUs[i]=p;
        pwm.setPWM(i,0,us2cnt(p));
        changed=true;
        if (p!=lastPrintedUs[i]){
          // Uncomment for debug spam:
          // Serial.printf("CH%u  pos=%4d us  target=%4d us  diff=%+4d us\n", i, p, t, (int)d);
          lastPrintedUs[i]=p;
        }
      }
    }
    if (changed) saveDirty=true;
    savePositionsIfNeeded();
    vTaskDelayUntil(&next, period);
  }
}

// TaskGame: runs the face-matching game state machine at GAME_CHECK_PERIOD_MS
static void TaskGame(void*){
  Serial.printf("\n>>> GAME task ready: %d faces\n", NUM_FACES);
  const TickType_t period=pdMS_TO_TICKS(GAME_CHECK_PERIOD_MS);
  TickType_t next=xTaskGetTickCount();
  uint32_t lastStatePush=0;

  for(;;){
    if(!gameActive){ vTaskDelayUntil(&next,period); continue; }
    uint32_t now=millis();

    const Face& F=faces[order[orderIdx]];
    uint32_t elapsed=now-faceStartMs;

    // Check if current positions fall inside the target face ranges
    if (faceMatched(F)){
      if(++stableCount>=STABLE_MATCH_FRAMES){
        stableCount=0;
        orderIdx++;
        sendGame(GAME_FACE_DONE, orderIdx, gameFaces);
        if (orderIdx >= gameFaces){
          gameStop(true); // WIN when all faces matched
        } else {
          faceStartMs=now;
          uint8_t sec = (uint8_t)min<uint32_t>(255, faceMs/1000U);
          sendGame(GAME_STATE, sec, order[orderIdx]);
        }
      }
    } else {
      stableCount=0;
    }

    // Timeout check -> lose
    if (elapsed >= faceMs && gameActive){
      gameStop(false); // LOSE (timeout)
    }

    // Push remaining time + current target face once per second
    if (gameActive && (now - lastStatePush) >= 1000) {
      uint32_t remain = (elapsed < faceMs) ? (faceMs - elapsed) : 0;
      uint32_t rs = remain / 1000U;
      if (rs > 255) rs = 255;
      sendGame(GAME_STATE, (uint8_t)rs, order[orderIdx]);
      lastStatePush = now;
    }

    vTaskDelayUntil(&next, period);
  }
}

/* ---------- RANDOMIZER (SIMULTANEOUS) ----------
   When triggered, assigns each channel a random target inside its per-servo limits,
   then (optionally) waits for all channels to settle or times out. */
#define RAND_SETTLE_US        10
#define RAND_TIMEOUT_MS     3000
#define RAND_REQUIRE_FREE      1
#define RAND_WAIT_FOR_SETTLE   1

static void TaskRandom(void*)
{
  for(;;){
    if (!randomTrigger){ vTaskDelay(pdMS_TO_TICKS(10)); continue; }
    randomTrigger = false;

    if (RAND_REQUIRE_FREE && gameActive){
      Serial.println(">>> RANDOMIZER ignored (Game running).");
      continue;
    }

    Serial.println(">>> RANDOMIZER (SIMULTANEOUS): start");

    int16_t rndTarget[N_SERVO];
    for (uint8_t i=0;i<N_SERVO;++i){
      rndTarget[i] = clampServoUs(i, randIn(lim[i].minUs, lim[i].maxUs));
    }
    for (uint8_t i=0;i<N_SERVO;++i){
      targetUs[i] = rndTarget[i];
      // Serial.printf("   ch%u -> %d us\n", i, rndTarget[i]);
    }

#if RAND_WAIT_FOR_SETTLE
    uint32_t t0 = millis();
    for(;;){
      if (RAND_REQUIRE_FREE && gameActive){
        Serial.println(">>> RANDOMIZER aborted (Game turned ON).");
        break;
      }
      bool allSet = true;
      for (uint8_t i=0;i<N_SERVO;++i){
        int16_t p = posUs[i];
        if (abs(p - rndTarget[i]) > RAND_SETTLE_US){ allSet=false; break; }
      }
      if (allSet) break;
      if ((millis() - t0) > RAND_TIMEOUT_MS){
        Serial.println(">>> RANDOMIZER: timeout before all settled.");
        break;
      }
      vTaskDelay(pdMS_TO_TICKS(20));
    }
#endif
    Serial.println(">>> RANDOMIZER (SIMULTANEOUS): done");
  }
}

// -------------------naration task-------------
// Small helpers to modify narration state atomically
static inline void StartNarration(){ narrIdx = 0; narrOn = true; }
static inline void StopNarration() { narrOn = false; }
static inline void ToggleNarration(){ if (narrOn) StopNarration(); else StartNarration(); }

// 50 Hz jaw updater (lets TaskSlew do smoothing/clamps)
// Reads successive entries from jawTrack[] while narrOn is true.
static void TaskNarrate(void*){
  const TickType_t period = pdMS_TO_TICKS(20);
  TickType_t next = xTaskGetTickCount();
  for(;;){
    vTaskDelayUntil(&next, period);
    if (!narrOn) continue;
    if (narrIdx >= jawFrames){ narrOn = false; continue; }
    uint16_t us = jawTrack[narrIdx++];
    // Set target; TaskSlew moves it smoothly:
    targetUs[10] = clampServoUs(10, (int16_t)us);
  }
}


/* ---------- Setup / Loop ---------- */
// setup(): hardware bring-up, NVS restore, ESP-NOW init, and task spawns
void setup(){
  Serial.begin(115200);
  for(int i=0;i<N_SERVO;++i){
    int16_t n = clampServoUs(i,1500);
    targetUs[i]=posUs[i]=lastPrintedUs[i]=n;
  }
#if USE_OE
  pinMode(OE_PIN,OUTPUT); digitalWrite(OE_PIN,HIGH); // keep outputs disabled during init
#endif

  Wire.begin(SDA_PIN,SCL_PIN);
  pwm.begin(); pwm.setPWMFreq(SERVO_FREQ);

  prefs.begin("servos", false);
  if (prefs.getBytesLength("pos")==sizeof(posUs)){
    // Restore last saved positions on boot (clamped to current limits)
    prefs.getBytes("pos", posUs, sizeof(posUs));
    for(int i=0;i<N_SERVO;++i){
      posUs[i]=clampServoUs(i,posUs[i]);
      targetUs[i]=lastPrintedUs[i]=posUs[i];
    }
    Serial.println("Restored positions from NVS.");
  } else {
    Serial.println("No saved positions; using neutral within limits.");
  }

  for(uint8_t i=0;i<N_SERVO;++i) pwm.setPWM(i,0,us2cnt(posUs[i]));
  delay(80);
#if USE_OE
  digitalWrite(OE_PIN,LOW); // enable outputs after initial write
#endif

  WiFi.mode(WIFI_STA);
  if (esp_now_init()!=ESP_OK){ Serial.println("ESP-NOW init failed"); for(;;); }
  ensurePeer(CONTROLLER_MAC);
  esp_now_register_recv_cb(onRx);

  // Allocate RX queue and spawn tasks on core 1 (mask device dedicated core)
  q=xQueueCreate(8,sizeof(Msg));
  xTaskCreatePinnedToCore(TaskRx,     "rx",     4096, nullptr, 3, nullptr, 1);
  xTaskCreatePinnedToCore(TaskSlew,   "slew",   4096, nullptr, 2, nullptr, 1);
  xTaskCreatePinnedToCore(TaskGame,   "game",   3072, nullptr, 1, nullptr, 1);
  xTaskCreatePinnedToCore(TaskRandom, "random", 2048, nullptr, 1, nullptr, 1);
  xTaskCreatePinnedToCore(TaskNarrate, "narr", 3072, nullptr, 1, nullptr, 1);

  Serial.println("Mask ready (Free on boot; long-press toggles modes; GAME_PREVIEW supported).");
}

// loop(): not used; all work is in FreeRTOS tasks
void loop(){}